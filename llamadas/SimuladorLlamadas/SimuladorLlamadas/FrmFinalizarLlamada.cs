...
{
    public partial class FrmFinalizarLlamada : Form
    {
        ...
        private void crear_controles()
        {
            ...
            lbl_titulo.Text = "Finalizar llamada";
            ...
            lbl_subtitulo.Text = "Registre la finalización de la llamada.";
            ...
            pnl_llamada.Size = new Size(252, 96);
            ...
            btn_finalizar_llamada = new Button();
            btn_finalizar_llamada.Text = "FINALIZAR";
            btn_finalizar_llamada.Location = new Point(35, 450);
            btn_finalizar_llamada.Size = new Size(252, 48);
        ...
        private void aplicar_bordes_redondeados()
        {
            ...
            TemaVisual.aplicar_borde_redondeado(btn_finalizar_llamada, 18);
        ...
        private void configurar_temporizador_hora()
        {
            ...
            this.FormClosed += frm_finalizar_llamada_form_closed;
        }
        ...
        private void frm_finalizar_llamada_form_closed(object? sender, FormClosedEventArgs e)
        {
        ...
        private void cargar_datos_telefono_origen()
        {
            ...
            cargar_llamada_actual_si_existe();
        }

        private void cargar_llamada_actual_si_existe()
        {
        ...
        private async void btn_finalizar_llamada_click(object? sender, EventArgs e)
        {
            string mensaje_json;
            ...
            try
            {
                mensaje_json = construir_json_finalizacion_llamada();
                ...
                limpiar_llamada_si_respuesta_ok(respuesta_servidor);

                txt_resultado.Text =
                    "Finalizacion de llamada" + Environment.NewLine +
                    ...
                    "Duracion real: " + ServicioFormato.segundos_a_reloj(EstadoLlamadaActual.duracion_real_segundos) +
        ...
        private void limpiar_llamada_si_respuesta_ok(string respuesta_servidor)
        {
            ...
            try
            {
                ...
                EstadoLlamadaActual.finalizar_llamada(respuesta_servidor);
            }
            catch
            {
                // Si la respuesta no viene como JSON válido,
                // no se limpia el estado interno.
            }
        }
        ...
        private bool validar_datos()
        {
            ...
            {
                mostrar_advertencia("Debe configurar una llave AES válida antes de finalizar una llamada.");
                return false;
            }
        ...
        private string construir_json_finalizacion_llamada()
        {
            FinalizacionLlamada finalizacion_llamada;
            JsonSerializerOptions opciones_json;
            string mensaje_json;

            finalizacion_llamada = new FinalizacionLlamada();
            finalizacion_llamada.transaccion = "finalizacion";

            finalizacion_llamada.telefono = ServicioAES.cifrar_texto(
                txt_numero_telefono.Text.Trim(),
                ConfiguracionSistema.llave_aes_base64
            );

            finalizacion_llamada.identificador_tel = ServicioAES.cifrar_texto(
                txt_identificador_telefono.Text.Trim(),
                ConfiguracionSistema.llave_aes_base64
            );

            finalizacion_llamada.identificador_chip = ServicioAES.cifrar_texto(
                txt_identificador_chip.Text.Trim(),
                ConfiguracionSistema.llave_aes_base64
            );

            finalizacion_llamada.coordenadas = txt_coordenadas.Text.Trim();
            finalizacion_llamada.telefono_destino = txt_telefono_destino.Text.Trim();

            opciones_json = new JsonSerializerOptions();
            opciones_json.WriteIndented = false;

            mensaje_json = JsonSerializer.Serialize(finalizacion_llamada, opciones_json);
